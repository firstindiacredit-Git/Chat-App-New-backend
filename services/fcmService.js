    ...
    try {
      ...
      if (admin.apps.length === 0) {
        ...
        const serviceAccount = {
          ...
          "client_email": process.env.FIREBASE_CLIENT_EMAIL || "firebase-adminsdk@chatapp-demo.iam.gserviceaccount.com",
        ...
        if (process.env.FIREBASE_PRIVATE_KEY && process.env.FIREBASE_PROJECT_ID) {
          ...
          console.log('📱 Firebase credentials not found - using development mode');
          console.log('📱 FCM notifications will be simulated via local notifications');
          return;
        }
      }
  ...
  async sendFCMNotification(userId, notificationData) {
    try {
      if (!this.isInitialized) {
        console.log('📱 FCM not initialized - using fallback local notification');
        return { success: false, message: 'FCM not initialized' };
      }

      const user = await User.findById(userId).select('name fcmTokens');
      if (!user || !user.fcmTokens || user.fcmTokens.length === 0) {
        console.log(`📱 No FCM tokens found for user: ${user?.name || 'Unknown'}`);
      ...
      for (const tokenData of user.fcmTokens) {
        try {
          ...
          console.error(`📱 Failed to send FCM notification to ${user.name}:`, error);
