...
class NotificationService {
  constructor() {
    ...
    const vapidKeys = {
      publicKey: process.env.VAPID_PUBLIC_KEY || 'BEl62iUYgUivxIkv69yViEuiBIa40HcCWLEaFyLFAaLqg7n9g7-d5FZz8_aD6bWGhRFdYZh6yBNgNnCW7y5Qd2c',
      privateKey: process.env.VAPID_PRIVATE_KEY || 'pWXBpBJp4z6Gn-R5qAXk5D9_c8lF1vG2sQ4nP8r6tY0'
    };
  ...
  async sendNotificationToUser(userId, notificationPayload) {
    try {
      ...
      if (!user || !user.pushSubscriptions || user.pushSubscriptions.length === 0) {
        console.log(`📱 No push subscriptions found for user: ${user?.name || 'Unknown'}`);
      ...
      for (const subscription of user.pushSubscriptions) {
        try {
          ...
          console.log(`📱 Push notification sent to ${user.name}:`, result.statusCode);
        } catch (error) {
   ...
   */
  async sendMessageNotification(receiverId, senderName, messageContent, chatId) {
    const payload = {
      title: senderName,
   ...
   */
  async sendCallNotification(receiverId, callerName, callType = 'voice', callId) {
    const payload = {
   ...
   */
  async sendSystemNotification(receiverId, title, message, type = 'info', data = {}) {
    const payload = {
      title: title,
      body: message,
      icon: '/vite.svg',
  ...
  async sendBulkNotifications(userIds, notificationPayload) {
    ...
    for (const userId of userIds) {
      const result = await this.sendNotificationToUser(userId, notificationPayload);
  ...
  async getNotificationStats() {
    try {
      ...
      const totalSubscriptions = users.reduce((sum, user) => 
        sum + (user.pushSubscriptions?.length || 0), 0
      );
